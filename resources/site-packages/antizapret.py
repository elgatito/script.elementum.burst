...
try:
    ...
    xbmc.log("[script.elementum.burst]: Could not make cache directory (%s): %s" % (CACHE_DIR, repr(e)), level=xbmc.LOGNOTICE)
    try:
        ...
        with shelf("antizapret.pac_config", ttl=CACHE) as pac:
            if not pac.get("value"):
                ...
                try:
                    ...
                    xbmc.log("[script.elementum.burst]: Fetching Antizapret PAC failed with code: %s" % (repr(e)), level=xbmc.LOGNOTICE)
                    data = ""
...
class AntizapretDetector:
    ...
    def detect(self, host="", scheme=""):
        ...
        xbmc.log("[script.elementum.burst]: Detect %s --- %s --- %s" % (host, len(self.config["domains"]), len(self.config["specials"])), level=xbmc.LOGNOTICE)
        return None
